<?php
	...
	mysqli_query($conn,"INSERT INTO movie (title,releaseDate,genre,plot,rating,inTheater) VALUES ('$title', '$releasedate','$genre','$plot','$rating','$intheater')")or die(mysqli_error($conn))? null : $all_query_ok=false;
	...
	$newartistchar=$_POST['newartistchar'];
		...
		{
		mysqli_query($conn,"INSERT INTO artist (firstName,lastName,dob,dod,nationality,Biography) VALUES ('$newartistfn[$i]', '$newartistln[$i]','$newartistdob[$i]','$newartistdod[$i]','$newartistnat[$i]','".mysql_real_escape_string($newartistbio[$i])."')")or die(mysqli_error($conn))? null : $all_query_ok=false; //inserting artist
		...
		if($all_query_ok=true){
		$uploadlocation_artistimg= uploadmyimage("newartistimg","$i","moviepics",$newartistfn[$i].$newartistln[$i]); //uploading image 
		mysqli_query($conn,"UPDATE artist SET image='$uploadlocation_artistimg' WHERE artistId='$artistId'")? null : $all_query_ok=false;
	...
	if(isset($_POST['review']) && isset($_SESSION['login_username'])){
	...
	//USER WATCHLIST 
	if(isset($_POST['fav_movieCode'])&& isset($_SESSION['login_username'])){
		$userid=$_SESSION['login_username'];
		$fav_movie=$_POST['fav_movieCode'];
		if($_POST['favourite']=="ADD TO WATCHLIST"){ 
			mysqli_query($conn,"INSERT INTO favourites(movieCode,userId) VALUES ('$fav_movie','$userid')")or die(mysqli_error($conn));
		...
		if($_POST['favourite']=="REMOVE FROM WATCHLIST"){
			mysqli_query($conn,"DELETE FROM favourites WHERE movieCode='$fav_movie' AND userId='$userid'")or die(mysqli_error($conn));
	...
	if(isset($_POST['register'])){
		...
		$all_query_ok=true;
		mysqli_query($conn,"INSERT INTO user(firstName,lastName,username,password) VALUES ('$firstName','$lastName','$username','$password')")or die(mysqli_error($conn))? null : $all_query_ok=false;
		$userid=mysqli_insert_id($conn);
		if($all_query_ok==false){
		}
		
		if($all_query_ok==true){
			$uploadlocation= uploadmyimage("userimg","0","userpics",$firstName.$lastName);
			mysqli_query($conn,"UPDATE user SET profilepic='$uploadlocation' WHERE userId='$userid'");
		}
		//CHECKING ADMENISTRATOR PREVILIGE
		if(isset($_SESSION['usertype'])&&$_SESSION['usertype']==0){
			if($_POST['usertype']==0){	
			$admin=$_POST['usertype'];
			mysqli_query($conn,"UPDATE user SET userType='$admin' WHERE userId='$userid'");//SETTING USER AS ADMINISTRATOR IF SET
			echo "<script>
					alert('NEW ADMIN REGISTRATION SUCESSFULL');
					window.location.href='".$_SESSION['redirect']."';
					</script>";
			exit();
			}else{
			echo "<script>
					alert('NEW USER SUCESSFULLY CREATED');
					window.location.href='".$_SESSION['redirect']."';
					</script>";
			exit();
			}
		...
		echo "<script>
					alert('REGISTRATION SUCESSFULL, PLEASE LOGIN NOW');
...
function uploadmyimage($filename,$index,$foldername,$newname){
 ...
 $ext=explode(".", $_FILES[$filename]["name"][$index]);
 $extension = end($ext);
